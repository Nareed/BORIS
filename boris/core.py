...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def show_plot_widget(self, plot_type: str, warning: bool = False) -> None:
        ...
        if plot_type == cfg.WAVEFORM_PLOT:
            if not self.actionShow_the_sound_waveform.isChecked():  # hide
                for media in self.waveform:
                    self.waveform[media].hidden = True
                return

            # all waveform widgets visible
            for media in self.waveform:
                self.waveform[media].hidden = False
    ...
    def plot_timer_out(self):
        ...
        if self.pj[cfg.OBSERVATIONS][self.observationId].get(cfg.VISUALIZE_WAVEFORM, False) and not self.pj[cfg.OBSERVATIONS][
            ...
            if not self.waveform[cfg.PLAYER1].visibleRegion().isEmpty():
                try:
                    wav_file_path = str(
                        Path(tmp_dir)
                ...
                if self.waveform[cfg.PLAYER1].wav_file_path == wav_file_path:
                    self.waveform[cfg.PLAYER1].plot_waveform(current_media_time, window_title=f"Waveform of {self.mem_media_name}")
                else:
                    r = self.waveform[cfg.PLAYER1].load_wav(wav_file_path)
                    if "error" not in r:
                        self.waveform[cfg.PLAYER1].plot_waveform(current_media_time, window_title=f"Waveform of {self.mem_media_name}")
                    else:
        ...
        if self.pj[cfg.OBSERVATIONS][self.observationId][cfg.MEDIA_INFO].get(cfg.PLAYER_PLOT_DISPLAY, {}):
            if not self.actionShow_the_sound_waveform.isChecked():
                for media in self.waveform:
                    self.waveform[media].hidden = True
                ...
                for media in self.waveform:
                    if media in current_media_path_list:
                        ...
                        if not self.waveform[media].hidden:
        ...
        if self.pj[cfg.OBSERVATIONS][self.observationId][cfg.MEDIA_INFO].get(cfg.PLAYER_PLOT_DISPLAY, {}):
            if not self.actionShow_spectrogram.isChecked():
                for media in self.spectro:
                    self.spectro[media].hidden = True
                ...
                for media in self.spectro:
                    if media in current_media_path_list:
                        ...
                        if not self.spectro[media].hidden:
    ...
    def current_media_path(self, player: int = 0) -> tuple[int | None, str]:
        ...
        playlist = self.dw_player[player].player.playlist
        playlist_pos = self.dw_player[player].player.playlist_pos
        if playlist is not None and playlist_pos is not None and playlist:
            current_media_path_ = playlist[playlist_pos]["filename"]
            current_playlist_index = playlist_pos

        return current_playlist_index, current_media_path_
